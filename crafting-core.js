...
function consume(inv,inputs){const next=Object.assign({},inv||{});for(const [id,n] of inputs||[])next[id]=Math.max(0,invQty(next,id)-n);return next;}
...
function canCraft(recipe,ctx={}){
 ...
 const ps=professionState(ctx,recipe.profession);const realm=ctx.playerRealm||0;
...
function chance(recipe,ctx={}){const ps=professionState(ctx,recipe.profession);const diff=(ps.grade||1)-recipe.grade;return clamp(.78+diff*.055+(ps.mastery||0)*.0014,.35,.99);}
function quality(recipe,ctx={},roll=Math.random()){
 const ps=professionState(ctx,recipe.profession);const score=(ps.mastery||0)*.0015+((ps.grade||1)-recipe.grade)*.045;
 const weights=[.55,.28,.13,.04];weights[0]=clamp(weights[0]-score,0.15,.7);weights[1]+=score*.45;weights[2]+=score*.35;weights[3]+=score*.2;
...
function craft(recipeId,ctx={}){
 const R=window.TuTienCraftingRecipes&&window.TuTienCraftingRecipes.BY_ID;const recipe=R&&R[recipeId];const check=canCraft(recipe,ctx);if(!check.ok)return check;
 const successChance=chance(recipe,ctx);const roll=typeof ctx.roll==='number'?ctx.roll:Math.random();const inventory=consume(ctx.inventory,recipe.inputs);
...
window.TuTienCraftingCore={version:1,Q,QN,hasInputs,canCraft,chance,quality,craft};
})();