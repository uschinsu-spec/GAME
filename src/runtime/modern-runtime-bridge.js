(()=>{'use strict';
let bound=false,unsubs=[],attempts=0,raf=0,sceneObserver=null,boundScene=null;
...
function bindScene(){if(bound)return true;const ctx=window.RuntimeContext&&window.RuntimeContext.discover();if(!ctx||!ctx.scene||!ctx.engine)return false;bound=true;boundScene=ctx.scene;
 ...
 window.GameEvents&&window.GameEvents.emit('modernRuntimeBound',{scene:ctx.scene,engine:ctx.engine,actors:(ctx.actors||[]).length,npcs:(ctx.npcs||[]).length,ownership:window.RuntimeOwnership&&window.RuntimeOwnership.snapshot()});return true;
}
