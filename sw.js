const BUILD='20260918-mobile-clarity-v5.3';
const CACHE={shell:'game-shell-v8',assets:'game-assets-v8',maps:'game-maps-v8'};
...
async function networkFirst(request,cacheName){const cache=await caches.open(cacheName);try{const response=await fetch(request);if(response&&response.ok)cache.put(request,response.clone());return response;}catch(error){const cached=await cache.match(request,{ignoreSearch:true});if(cached)return cached;throw error;}}
...
self.addEventListener('fetch',event=>{
 ...
 if(request.mode==='navigate'||/\/index\.html$/.test(url.pathname)||/\/build\.json$/.test(url.pathname))return event.respondWith(networkFirst(request,CACHE.shell));
 ...
 if(/\.(js|css|json|webmanifest)$/i.test(url.pathname))return event.respondWith(staleWhileRevalidate(request,CACHE.shell));
