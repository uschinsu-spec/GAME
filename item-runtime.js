...
function ensure(S){
 ...
 if(S.inventory){
  ...
  if(!Array.isArray(S.inventory.artifacts))S.inventory.artifacts=[];
  S.inventory.materials=S.inventory.materials||{};
  x.materials=S.inventory.materials;
  if(typeof x.materials.refineStone!=='number')x.materials.refineStone=20;
  if(typeof x.materials.spiritIron!=='number')x.materials.spiritIron=20;
 }else if(!Array.isArray(x.inventory))x.inventory=[];
...
function migrateLegacy(S,x){
 ...
 Object.entries(map).forEach(([oldSlot,newSlot])=>{
  ...
  const it=createGear(newSlot,1,'trung','Vô');it.name=String(legacy[oldSlot]);pushItem(S,it);x.equipment[newSlot]=it.id;
 });
}
...
function inventoryItems(S=STATE){const x=ensure(S);return S&&S.inventory?[...(S.inventory.equipment||[]),...(S.inventory.artifacts||[])]:x.inventory;}
function pushItem(S,it){if(S&&S.inventory){const key=it.kind==='artifact'?'artifacts':'equipment';S.inventory[key].push(it);}else ensure(S).inventory.push(it);return it;}
function removeItem(S,id){if(S&&S.inventory){for(const key of ['equipment','artifacts']){const list=S.inventory[key]||[],i=list.findIndex(v=>v&&v.id===id);if(i>=0)return list.splice(i,1)[0];}}else{const list=ensure(S).inventory,i=list.findIndex(v=>v.id===id);if(i>=0)return list.splice(i,1)[0];}return null;}
function find(id){return inventoryItems().find(x=>x.id===id)||null;}
...
function totalBonuses(S){
 ...
 const items=inventoryItems(S);for(const id of ids){const it=items.find(z=>z.id===id);if(!it)continue;const qm=qualityDef(it.quality).mult;
  if(it.kind==='gear'){
...
function save(){if(window.SaveService)return window.SaveService.markDirty('items');try{localStorage.setItem('tutien_chilo_save_v2',JSON.stringify(STATE));}catch(e){}}
...
function addDrop(it){pushItem(STATE,it);window.GameEvents&&window.GameEvents.emit('itemAdded',{item:it,id:it.id,qty:1,bucket:it.kind==='artifact'?'artifacts':'equipment'});save();toast(`✨ Nhặt ${gradeDef(it.grade).name} · ${qualityDef(it.quality).name} · ${it.name}`);}
...
function craftGear(slot,grade,element='Vô'){if(!STATE)return null;grade=clamp(grade,1,5);if(currentRealm()<grade-1)return null;const x=ensure(STATE),cost=Math.round(4*Math.pow(2,grade-1));if((x.materials.spiritIron||0)<cost)return null;x.materials.spiritIron-=cost;const it=createGear(slot,grade,rollQuality(false),element);pushItem(STATE,it);save();return it;}
function refineThree(ids){if(!STATE||!Array.isArray(ids)||ids.length!==3)return null;const arr=ids.map(find);if(arr.some(v=>!v)||new Set(arr.map(v=>`${v.kind}|${v.grade}|${v.quality}`)).size!==1)return null;const qi=QUALITY_INDEX[arr[0].quality];if(qi>=3)return null;const next=M.QUALITIES[qi+1].id;ids.forEach(id=>removeItem(STATE,id));const src=arr[0],it=src.kind==='gear'?createGear(src.slot,src.grade,next,src.element):createArtifact(src.grade,next,src.element);pushItem(STATE,it);save();return it;}
...
function renderInventory(){if(!STATE)return;const body=document.querySelector('#panelBody'),title=document.querySelector('#panelTitle');if(!body)return;const x=ensure(STATE);if(title)title.textContent='Túi Đồ · Item';const inv=inventoryItems().slice().sort((a,b)=>b.grade-a.grade-(QUALITY_INDEX[b.quality]-QUALITY_INDEX[a.quality]));body.innerHTML=`<div class="card"><b>🎒 Hệ ITEM Tu Tiên</b><p>Nhất→Ngũ Phẩm · Hạ→Cực phẩm · khóa theo cảnh giới.</p><div class="stat"><span>Linh Thiết</span><b>${x.materials.spiritIron||0}</b></div><div class="stat"><span>Đá Tinh Luyện</span><b>${x.materials.refineStone||0}</b></div></div><div class="cards">${inv.length?inv.map(i=>card(i,x)).join(''):'<div class="card"><p>Chưa có trang bị/pháp bảo mới. Diệt quái, Boss hoặc luyện khí để nhận.</p></div>'}</div>`;bind();}
...
function attachState(S){STATE=S;ensure(S);applyBonuses(S);}
function onKilled(meta){if(!meta||!meta.state)return;STATE=meta.state;const chance=meta.boss?.80:.12;if(Math.random()<chance)rollDrop(STATE,!!meta.boss);if(Math.random()<.18){ensure(STATE).materials.spiritIron+=(meta.boss?3:1);}save();}
if(window.GameEvents){window.GameEvents.on('stateReady',({state})=>attachState(state));window.GameEvents.on('enemyKilled',onKilled);window.GameEvents.on('bossKilled',onKilled);}
window.TuTienItems={ensure,attachState,createGear,createArtifact,equip,unequip,rollDrop,craftGear,refineThree,totalBonuses,renderInventory,renderEquipment,getState:()=>STATE&&ensure(STATE)};
})();
