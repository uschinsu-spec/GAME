...
const regions=[
  {id:'thanh_van_thon',name:'Thanh Vân Thôn',kind:'Thôn',min:1,max:12,color:'#668071',enemy:['wolf','fox']},
  {id:'linh_son_ngoai_vi',name:'Linh Sơn Ngoại Vi',kind:'Man Hoang',min:8,max:20,color:'#536f58',enemy:['wolf','golem']},
  {id:'bach_ngoc_thanh',name:'Bạch Ngọc Thành',kind:'Thành',min:15,max:30,color:'#72756b',enemy:['fox','shadow']},
  {id:'thanh_van_tong',name:'Thanh Vân Tông',kind:'Tông Môn',min:25,max:45,color:'#4f6d76',enemy:['golem','shadow']},
  {id:'van_dam_sa_mac',name:'Vạn Dặm Sa Mạc',kind:'Man Hoang',min:40,max:65,color:'#88765d',enemy:['golem','wolf']},
  {id:'yeu_vuc',name:'Yêu Vực',kind:'Man Hoang',min:60,max:90,color:'#5b4b62',enemy:['shadow','wolf','fox']},
  {id:'cam_dia_han_uyen',name:'Cấm Địa Hàn Uyên',kind:'Cấm Địa',min:85,max:120,color:'#4b6873',enemy:['shadow','golem']},
  {id:'ma_vuc',name:'Ma Vực',kind:'Cấm Địa',min:110,max:160,color:'#5d4248',enemy:['shadow','golem','wolf']}
];
...
const MAP_CONFIGS={
  thanh_van_thon:{
    ...
    groundScale:8,
    river:{enabled:true,color:'#477e83',x:-32,rotY:0.12,width:10,height:80},
    trees:[
      {file:'assets/maps/thanh_van_thon/trees/tree_01.png',width:4.8,height:5.8},
      {file:'assets/maps/thanh_van_thon/trees/tree_02.png',width:4.6,height:5.6},
      {file:'assets/maps/thanh_van_thon/trees/tree_03.png',width:4.4,height:5.4}
    ],
    rocks:[
      {file:'assets/maps/thanh_van_thon/rocks/rock_01.png',width:2.4,height:2.4},
      {file:'assets/maps/thanh_van_thon/rocks/rock_02.png',width:2.0,height:2.0}
    ],
    decor:[
      {file:'assets/maps/thanh_van_thon/decor/lantern.png',width:1.6,height:2.8},
      {file:'assets/maps/thanh_van_thon/decor/grass_01.png',width:1.3,height:1.3}
    ],
    treeCount:44,
    rockCount:28,
    grassCount:26,
...
function makeSpriteMaterial(url, key){
  ...
  m.emissiveColor=new BABYLON.Color3(1.15,1.15,1.15);
  m.specularColor=BABYLON.Color3.Black();
...
function preloadPlayerMaterials(){
  ['idle','run','attack'].forEach(state=>{
    ...
    for(let i=0; i<n; i++){
      ...
      let url = `assets/player/right/${state}/${s}.png?v=3`;
      ...
      m.emissiveTexture = t;
      m.useAlphaFromDiffuseTexture = true;
      m.emissiveColor = new BABYLON.Color3(1.2, 1.2, 1.2);
      m.specularColor = BABYLON.Color3.Black();
      m.disableLighting = true;
      m.backFaceCulling = false;
      m.transparencyMode = BABYLON.Material.MATERIAL_ALPHABLEND;
...
function spriteFrames(type){
  let n = 8, arr = [];
  for(let i=0; i<n; i++){
    arr.push(makeSpriteMaterial(`assets/sprites/${type}_${String(i).padStart(2,'0')}.png?v=3`, type+i));
  }
  return arr;
}
...
function loadMap(regionIdx){
  ...
  scene.fogStart=cfg.fogStart||34;
  scene.fogEnd=cfg.fogEnd||62;
  ...
  if(worldRiver){
    if(cfg.river&&cfg.river.enabled){
      worldRiver.setEnabled(true);
      worldRiver.position.x=cfg.river.x;
      worldRiver.rotation.y=cfg.river.rotY||0.12;
      let rm=mat('river_'+reg.id,cfg.river.color||'#477e83',0.82);
      rm.emissiveColor=new BABYLON.Color3(0.04,0.12,0.13);
      worldRiver.material=rm;
    }else{
      worldRiver.setEnabled(false);
    }
  }

  // Cây cối PNG (Trees)
  if(cfg.trees&&cfg.trees.length>0){
    for(let i=0;i<(cfg.treeCount||40);i++){
      let x=rnd(-45,45),z=rnd(-45,45);
      if(Math.abs(x)<12&&Math.abs(z)<15)continue;
      if(cfg.river&&cfg.river.enabled&&Math.abs(x-cfg.river.x)<6)continue;
  ...
  if(cfg.rocks&&cfg.rocks.length>0){
    for(let i=0;i<(cfg.rockCount||24);i++){
      let x=rnd(-46,46),z=rnd(-46,46);
      if(Math.abs(x)<8&&Math.abs(z)<8)continue;
      if(cfg.river&&cfg.river.enabled&&Math.abs(x-cfg.river.x)<5)continue;
  ...
  // Linh thảo / Bụi cỏ PNG (Grass)
  if(cfg.decor&&cfg.decor[1]){
    let grassDef=cfg.decor[1];
    for(let i=0;i<(cfg.grassCount||20);i++){
      let x=rnd(-44,44),z=rnd(-44,44);
      spawnMapProp('grass_'+i,grassDef,x,z,0.25);
    }
  }
}
...
function createWorld(){
  ...
  worldGround=BABYLON.MeshBuilder.CreateGround('ground',{width:100,height:100,subdivisions:2},scene);

  worldRiver=BABYLON.MeshBuilder.CreateGround('river',{width:10,height:80},scene);
  worldRiver.position.y=.015;
...
function createPlayer(){
  player={
    x:0,z:2,
    mesh:makeBillboard('player','player',3.4,0,2),
    facing:'right',
    state:'idle',
    frame:0,
    animTimer:0,
  ...
  player.shadow.position.set(player.x,0.019,player.z);
...
function spawnPack(){
  ...
  let ang=rnd(0,Math.PI*2),r=rnd(14,25);
  for(let i=0;i<(type==='wolf'?3:2);i++){
    makeActor(type,clamp(player.x+Math.cos(ang)*r+rnd(-3,3),-44,44),clamp(player.z+Math.sin(ang)*r+rnd(-3,3),-44,44),Math.random()<.08);
  }
}

function spawnBoss(){
  if(boss||S.questKills<20)return;
  boss=makeActor('shadow',clamp(player.x+12,-40,40),clamp(player.z+8,-40,40),true);
...
function useDash(){
  ...
  player.x=clamp(player.x+mx*6,-46,46);
  player.z=clamp(player.z+mz*6,-46,46);
  player.invuln=0.45;
...
function updateActor(a,dt){
  ...
  if(a.frameT>.11){
    a.frameT=0;
    a.frame=(a.frame+1)%8;
    if(spriteMats[a.type+a.frame])a.mesh.material=spriteMats[a.type+a.frame];
  }
...
function playerDeath(){
  ...
  setTimeout(()=>{
    ...
    paused=false;
    toast('☯ Đã hồi sinh tại Thanh Vân Thôn');
    updateHUD();
  },2200);
}
...
function updatePlayer(dt){
  ...
  if(l>.05){
    ...
    let sp=5.4;
    player.x=clamp(player.x+nx*sp*dt,-46,46);
    player.z=clamp(player.z+nz*sp*dt,-46,46);
  }
...
function drawMini(){
  let c=$('#mini'),x=c.getContext('2d');
  x.clearRect(0,0,c.width,c.height);
  x.fillStyle='#44554b';
  x.fillRect(0,0,c.width,c.height);
  x.strokeStyle='#a49b80';
  x.lineWidth=8;
  x.beginPath();
  x.moveTo(20,110);
  x.quadraticCurveTo(90,45,130,0);
  x.stroke();
  for(let a of actors){
    if(a.dead)continue;
    x.fillStyle=a===boss?'#ffcf45':'#d94a43';
    x.beginPath();
    x.arc((a.x+50)/100*c.width,(a.z+50)/100*c.height,a===boss?4:2,0,7);
    x.fill();
  }
  x.fillStyle='#75e9ff';
  x.beginPath();
  x.arc((player.x+50)/100*c.width,(player.z+50)/100*c.height,3.5,0,7);
  x.fill();
}
...
async function init(){
  ...
  createPlayer();
  for(let t of ['wolf','fox','golem','shadow'])spriteFrames(t);
